...
/* Function:  BaseMatrix_alloc_matrix_specials_score_offset(len_point,len_array,len_spec_point,len_spec_array)
 *
 * Descrip:    This function both allocates a matrix and a store
 *             for integer underflow/overflow
 *
 *
 * Arg:             len_point [UNKN ] Undocumented argument [int]
 * Arg:             len_array [UNKN ] Undocumented argument [int]
 * Arg:        len_spec_point [UNKN ] Undocumented argument [int]
 * Arg:        len_spec_array [UNKN ] Undocumented argument [int]
 *
 * Return [UNKN ]  Undocumented return value [BaseMatrix *]
 *
 */
# line 179 "basematrix.dy"
...
BaseMatrix * BaseMatrix_alloc_matrix(int leni,int lenj) 
{
    ...
    if((out->matrix = (BaseMatrixIntType **) ckcalloc (leni,sizeof(BaseMatrixIntType *))) == NULL)   {  
    ...
    for(i=0;i<leni;i++)  {  
      out->matrix[i] = (BaseMatrixIntType *) ckcalloc (lenj,sizeof(BaseMatrixIntType ));     
...
boolean expand_BaseMatrix(BaseMatrix * obj,int leni,int lenj) 
{
    ...
    if( obj->maxleni < leni )    {  
      if( (obj->matrix=(BaseMatrixIntType **) ckrealloc (obj->matrix,sizeof(BaseMatrixIntType *)*leni)) == NULL) 
        return FALSE;    
    ...
    for(i=0;i<obj->leni;i++) {  
      if((obj->matrix[i] = (BaseMatrixIntType *) realloc (obj->matrix[i],sizeof(BaseMatrixIntType ) * actualj)) == NULL) 
        return FALSE;    
      }  
    return TRUE;     
}    
