...
Search_Return_Type search_ProteinSW(DBSearchImpl * dbsi,Hscore * out,ProteinDB* querydb,ProteinDB* targetdb ,CompMat* comp,int gap,int ext) 
{
    ...
    if( dbsi->trace_level > 0 )  
      warn("Although you are asking at run-time for database tracing, the ProteinSW matrix was not compiled with database tracing. No tracing will be made");    
    switch(dbsi->type)   { /*switch on implementation*/ 
      ...
      case DBSearchImpl_Pthreads :   
        warn("This matrix ProteinSW was not dyc compiled with thread support");  
        return SEARCH_ERROR; 
      default :  
...
/* Function:  alloc_naive_vector_ProteinSW(query,target,comp,gap,ext)
 *
 * Descrip:    allocating for naive vectorisation ProteinSW
 *
 *
 ...
 *
 * Return [UNKN ]  Undocumented return value [struct naive_vector_ProteinSW *]
 *
 */
...
{
    struct naive_vector_ProteinSW * out; 
    int total_depth; 
    int j;   


    out->shift = calloc(2,sizeof(int*)); 
    out->non_shift = calloc(2,sizeof(int*)); 


    total_depth = 8 * (int)(1+floor((query->seq->len / 8))) ;    
    for(j=0;j<8;j++) {  
      out->shift[j] = calloc(total_depth*3,sizeof(int)); 
      out->non_shift[j] = calloc(total_depth*3,sizeof(int)); 
      }  
    return out;  
}    
