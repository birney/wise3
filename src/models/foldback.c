...
int main(int argc,char ** argv)
{
  ...
  stmat = identity_DnaMatrix(stmatch,stmismatch);
  ...
  if( one == NULL || two == NULL || mat == NULL ) {
    fatal("Some objects are null - cannot build alignment");
  }

  fprintf(stderr,"About to call PackAln with %d and %d\n",one->len,two->len);

  pal = PackAln_bestmemory_FoldBackMat(cone,ctwo,mat,stmat,-gap,-ext,-gap,-ext,-lgap,-lext,-stgap,dpenv,dpri);

  alb = convert_PackAln_to_AlnBlock_FoldBackMat(pal);
