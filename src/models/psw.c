...
void show_help(FILE * ofp)
{
  fprintf(ofp,"\npsw <options> seq1 seq2\nBoth sequences in fasta format\n"
	  ...
	  "\t-f show fancy output\n"
          ...
          "\t-n length of name block (default 15)\n"
          "\t-w length of alignment block (default 50)\n"
...
int main(int argc,char ** argv)
{
  ...
  strip_out_integer_argument(&argc,argv,"n",&name_block);
  strip_out_integer_argument(&argc,argv,"w",&alignment_block);
...
/*  show_DPEnvelope(dpenv,stderr); */
  ...
  if( show_fancy_output == TRUE ) {
    write_pretty_seq_align(alb,query,target,name_block,alignment_block,stdout);
    puts("//\n");
  }
